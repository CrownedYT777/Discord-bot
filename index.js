...
client.on('guildMemberAdd', async (member) => {
    ...
    try {
        await channel.send({ content: `Welcome to the server, ${member}!`, files: [welcomeImageUrl] })
            .catch(err => {
                console.error(`❌ Failed to send welcome image with file:`, err);
                // Fallback to just sending the URL if file attachment fails
        ...
        // Attempt to send a basic welcome message as fallback
        try {
            await channel.send(`Welcome to the server, ${member}!`);
        } catch (innerError) {
            console.error(`❌ Failed to send fallback welcome message:`, innerError);
        }
    }
});
