...
client.on('messageCreate', async message => {
    ...
    // Ensure message.member exists before continuing
    if (!message.member) {
        console.log(`Message received but message.member is null for user ${message.author.tag} in guild ${message.guild.id}`);
        return;
    }
    ...
    if (blacklistData[guildId] && blacklistData[guildId].length > 0) {
        ...
        // Check if moderator system is set up for this guild
        if (moderatorsData.moderators[guildId]) {
            // Skip moderation for the bot's own messages
            if (message.author.id === client.user.id) {
                isWhitelisted = true;
            }
            // Check if user is directly whitelisted
            else if (moderatorsData.moderators[guildId].users && moderatorsData.moderators[guildId].users.includes(message.author.id)) {
                isWhitelisted = true;
            } else if (moderatorsData.moderators[guildId].roles && message.member.roles) {
